---
title: getAll()
description: Retrieves a paginated list of questions with comprehensive filtering, sorting, and search options. Works in both public and team contexts.
---

# getAll()

Retrieves a paginated list of questions with comprehensive filtering, sorting, and search options. Works in both public and team contexts.
...
// Compare team vs public activity
const [teamRecent, publicRecent] = await Promise.all([
...
console.log(`- Public recent questions: ${publicRecent.totalCount}`);
```
...
```typescript
import StackOverflowSDK, { AuthenticationError, ValidationError, ForbiddenError } from 'stack-overflow-sdk';
...
- **Team vs Public Context**: Questions are retrieved from team context when using `sdk.forTeam()`, otherwise from public context
