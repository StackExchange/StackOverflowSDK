...
async function checkProfileCompleteness() {
  ...
  Object.entries(completeness.checks).forEach(([key, check]) => {
    if (check.passed) {
      ...
      console.log(`✓ ${key.replace('has', '')} ${status}`);
    }
  });
...
```typescript
import StackOverflowSDK, { AuthenticationError, TokenExpiredError, ForbiddenError } from '@stackoverflow/teams-sdk';
